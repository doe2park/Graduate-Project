...
def package(source,output,panoramas,station_id,mobile_budget=350000,iterations=None):
    ...
    output.mkdir(parents=True);shutil.copyfile(source,output/'trained.ply');write_ply(output/'mobile.ply',mobile)
    report={'status':'experimental-trained','frame':'scan-y-up-metres','file':'trained.ply','mobileFile':'mobile.ply','splatCount':len(data),'mobileSplatCount':len(mobile),'camera':camera,'maxDistance':120,
      'provenance':{'trainingIterations':iterations,'sourceCheckpointSha256':sha(source),'desktopSha256':sha(output/'trained.ply'),'mobileSha256':sha(output/'mobile.ply'),'desktopBytes':(output/'trained.ply').stat().st_size,'mobileBytes':(output/'mobile.ply').stat().st_size,'initialStation':station_id,'mobileMethod':'SH0; opacity >=0.02; 15cm cell coverage and opacity budget. Lossy visual derivative; no change to source checkpoint.','geometryScope':'Trained appearance, not surveyed geometry or device identity. Same Cupix scan frame must be verified.'}}
    (output/'viewer.json').write_text(json.dumps(report,indent=2));return report

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--input',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--panoramas',type=Path,required=True);p.add_argument('--station-id',required=True);p.add_argument('--mobile-budget',type=int,default=350000);p.add_argument('--iterations',type=int);a=p.parse_args();print(json.dumps(package(a.input,a.output,a.panoramas,a.station_id,a.mobile_budget,a.iterations),indent=2))
