...
`scan-compare.html` is the public comparison entry point. It defaults to a `scan-photo.html` captured panorama pane and a `scan-bim.html` BIM pane (the scan selector also offers the original mesh) and applies the accepted rigid transform in `alignment-analysis/accepted-registration.json`. Both panes may drive the camera; position, orientation, field of view and zoom are mapped between frames.

Photo mode renders 100 recovered E57 panoramas without Cupix login or its viewer; mesh mode renders the two original scan GLBs. See [captured panorama binding and navigation](CAPTURED_PANORAMA_VIEWER.md). `scan-assets/provenance.json` records byte counts and SHA-256 hashes. No geometry recompression, node renaming or modification of the machine-written data branch was performed. The 217 numeric camera positions and 100 recovered photo/pose bindings are published; raw capture/account inventory is excluded.
